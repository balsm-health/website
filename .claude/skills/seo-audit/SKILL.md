...
1. **Render homepage**: use `"/Volumes/Dev/Balsm/website/.claude/skills/seo/bin/claude-seo" run render_page.py <url> --mode auto --json` to capture raw HTML, rendered HTML, extracted text, SPA status, and accessibility data when needed
...
4. **Delegate to subagents** (if available, otherwise run inline sequentially):
   ...
   - `seo-google` -- CWV field data (CrUX), URL indexation (GSC), organic traffic (GA4) (spawn when Google API credentials detected via `"/Volumes/Dev/Balsm/website/.claude/skills/seo/bin/claude-seo" run google_auth.py --check`)
   - `seo-backlinks` -- Backlink profile data: DA/PA, referring domains, anchor text, toxic links (spawn when Moz or Bing API credentials detected via `"/Volumes/Dev/Balsm/website/.claude/skills/seo/bin/claude-seo" run backlinks_auth.py --check`, or always include Common Crawl domain-level metrics)
   ...
   - `seo-drift` -- Drift analysis: compare against stored baseline (spawn when drift baseline exists for the URL via `"/Volumes/Dev/Balsm/website/.claude/skills/seo/bin/claude-seo" run drift_history.py <url>`)
